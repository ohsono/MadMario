...
def _run(cfg: "Config") -> None:  # noqa: F821
    from madmario.config import Config
    from madmario.environment import make_env
    from madmario.agent import Mario
    from madmario.metrics import MetricLogger
    from madmario.mcp_server import update_state

    _seed(cfg.train.seed)

    if cfg.multi_agent.enabled:
        from madmario.multi_agent import run_multi_agent
    ...
    if cfg.autonomous.enabled:
        from madmario.autonomous import SelfImprovementLoop
